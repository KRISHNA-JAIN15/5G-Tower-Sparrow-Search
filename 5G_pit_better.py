...
def coverage_ratio(base_stations, area_size, grid_size, Rs, Re=1.0, lambda_param=1.0,
                    ...
                    X=None, Y=None, weight_pit=3.0, cost_weight=1.5):
    """Calculate coverage ratio with stronger penalties for large radii and cost"""
    ...
    # Standard penalties for placement and clustering (unchanged)
    placement_penalty = 0
    ...
    # INCREASED RADIUS PENALTY
    characteristic_length = np.sqrt(area_size[0] * area_size[1])
    relative_radius = Rs / characteristic_length
    
    # Lower threshold to penalize even smaller radii
    radius_threshold = 0.08  # 8% of characteristic length
    
    # Increased base penalty for all radii
    base_radius_penalty = 0.8 * (Rs ** 3) / characteristic_length
    
    # Much stronger additional penalty that grows more rapidly with radius
    additional_penalty = 0.0
    if relative_radius > radius_threshold:
        # More aggressive exponential growth for penalty as radius increases
        additional_penalty = 3.0 * np.exp(4.0 * (relative_radius - radius_threshold))
    
    # Coverage efficiency component
    total_area = area_size[0] * area_size[1]
    theoretical_max_coverage = min(1.0, len(base_stations) * np.pi * Rs**2 / total_area)
    efficiency_score = total_coverage / theoretical_max_coverage if theoretical_max_coverage > 0 else 0
    
    # Combine base penalty with additional penalty, adjusted by efficiency
    radius_penalty = base_radius_penalty + additional_penalty * (1.0 - efficiency_score)
    
    # ENHANCED COST FUNCTION PENALTY
    # Base cost for each station 
    base_cost = len(base_stations) * 1.0  # Base cost per station
    
    # Cost increases more aggressively with radius (cubic relationship)
    # This makes larger radii significantly more expensive
    radius_cost = len(base_stations) * Rs * np.math.exp(Rs)
    
    # Add a progressive cost tier for larger radii
    tier_threshold = 0.15 * characteristic_length
    if Rs > tier_threshold:
        # Additional premium for large radius stations
        premium_cost = len(base_stations) * 2.0 * ((Rs - tier_threshold) ** 5)
        radius_cost += premium_cost
    
    # Total cost combining all components
    total_cost = base_cost + radius_cost
    
    # Increased cost weight and scaling
    scaled_cost = cost_weight * total_cost / (characteristic_length ** 2)
    
    min_coverage_reward = 0
    if total_coverage < 0.85:
        min_coverage_reward = -5.0 * (0.85 - total_coverage)  # penalize low coverage more directly

    
    # Combined fitness formula with increased radius penalty and cost
    return (3.5 * total_coverage + 5.0 * pit_coverage) - \
           (1.0 * placement_penalty) - \
           (2.0 * clustering_penalty) - \
           (2.5 * radius_penalty) - \
           scaled_cost + min_coverage_reward
