...
def coverage_ratio_multi_region(base_stations, area_size, grid_size, Rs, Re=1.0, lambda_param=1.0,
    ...
    return (
        ...
        station_reward +
        efficiency_reward -
        3.0 * placement_penalty -
        25.0 * clustering_penalty -
        3.5 * radius_penalty -
        scaled_cost +
        min_coverage_reward
    )
...
if __name__ == "__main__":
    print("Running Parameter Optimization for Base Station Placement with Multiple Pit Sizes...")
    ...
    early_stop = 50
    num_sparrows = 100
    num_pits = 4 # Number of pit regions to generate
    pit_sizes = [1.0, 1.5 , 2.5 , 2.5]  # Define different sizes for each pit

    # Parameters to optimize
    station_range = range(2, 11)  # Try 2 to 10 stations
    rs_values = [round(1.0 + i * 0.2, 1) for i in range(21)]  # Rs from 1.0 to 5.0 in 0.2 steps
